...
func (l *Listener) ListenOnConnection(conn *connections.ParticipantConnection) {
	defer func() {
		if r := recover(); r != nil {
	...
	for {
		...
		if err != nil {
			if os.IsTimeout(err) {
				l.logger.Printf("participant %s timeout\n", conn.Ws.RemoteAddr())
				conn.Close()
				return
			}
			...
			conn.ResetReadTimer()
			l.logger.Printf("new message from participant %s, payload: \"%s\"\n",
				conn.Ws.RemoteAddr(),
				request.Payload,
			)
